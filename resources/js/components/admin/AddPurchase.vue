                                ...
                                >
                                    <div class="card-body px-sm-3 px-0">
                                        ...
                                        <div
                                            class="col-md-12"
                                            v-if="
                                                combination_products.length > 0
                                            "
                                        >
                                            <label class="text-body"
                                                >Select Variation Product</label
                                            >
                                            <fieldset
                                                class="form-group mb-3 d-flex"
                                            >
                                        ...
                                        >
                                            <div class="table-responsive">
                                                <table
                                                    class="table table-striped text-body"
                                                >
                                                    <thead>
                                                        <tr>
                                                            <th
                                                                class="border-0 header-heading"
                                                                scope="col"
                                                            >
                                                                Name
                                                            </th>
                                                    ...
                                                    <tbody>
                                                        <tr
                                                            class=""
                                                            v-for="(selectedProduct,
                                                            index) in selectedProducts"
                                                        >
                                                            <td>
                                                                {{
                                                            ...
                                                            <input
                                                                type="hidden"
                                                                :set="
                                                                    (purchase.product_combination_id[
                                                                        index
                                                                    ] =
    ...
    methods: {
        ...
        computeValues: function() {
            if (this.purchase.qty.length == this.purchase.price.length) {
                ...
                this.purchase.total_amount =
                    subTotal - this.purchase.discount_amount;
        ...
        getQuotation() {
            ...
            axios
                ...
                .then(res => {
                    if (res.data.status == "Success") {
                        this.purchase.warehouse_id = res.data.data.warehouse
                            ? res.data.data.warehouse.warehouse_id
                            : "";
                        // this.purchase.customer_id = res.data.data.customer.customer_id;
                        for (
                            var i = 0;
                            i < res.data.data.quotationDetail.length;
                            i++
                        ) {
                            var quotationDetail =
                                res.data.data.quotationDetail[i];
                            ...
                            if (
                                quotationDetail.product.product_type ==
                                "variable"
                            ) {
                                ...
                                if (combination_detail.length > 0) {
                                    ...
                                    for (
                                        var i = 0;
                                        i < combination_detail.length;
                                        i++
                                    ) {
                                        if (
                                            quotationDetail.product_combination_id ==
                                            combination_detail[i]
                                                .product_combination_id
                                        ) {
                                            arr.product_price =
                                                combination_detail[i].price;
        ...
        addProduct(selectedOption, id, quotationDetail = {}) {
            console.log(
                quotationDetail,
                "add product",
                quotationDetail.hasOwnProperty("qty")
            );
            this.errors = new ErrorHandling();
            this.$parent.loading = true;
            var token = localStorage.getItem("token");
                ...
                .then(res => {
                    if (res.data.status == "Success") {
                        ...
                        if (res.data.data.product_type == "simple") {
                            ...
                            arr.qty = quotationDetail.hasOwnProperty("qty")
                                ?parseInt( quotationDetail.qty)
                                : 1;
                            arr.stock_status = "";
                            arr.warehouse = "";
                            arr.price = quotationDetail.hasOwnProperty("price")
                                ? quotationDetail.price
                                : "";
                            this.purchase.qty.push(
                                quotationDetail.hasOwnProperty("qty")
                                    ?parseInt( quotationDetail.qty)
                                    : 1
                            );
                            this.purchase.price.push(
                                quotationDetail.hasOwnProperty("qty")
                                    ? quotationDetail.price
                                    : ""
                            );
                            ...
                            if (res.data.data.combination_detail.length > 0) {
                                for (
                                    ...
                                    arr.product_combination_id =
                                        res.data.data.combination_detail[
                                            i
                                        ].product_combination_id;
                                    var combination_name = "";
                                    if (
                                        res.data.data.combination_detail[i]
                                            .combination.length > 0
                                    ) {
                                        for (
                                            var j = 0;
        ...
        addCombinationProduct(selectedOption, id,quotationDetail={}) {
            this.selectedProducts.push(selectedOption);
            this.purchase.qty.push(
                quotationDetail.hasOwnProperty("qty") ?parseInt( quotationDetail.qty) : 1
            );
            this.purchase.price.push(
                quotationDetail.hasOwnProperty("qty")
                    ? quotationDetail.price
        ...
        removeItem(removedOption, id) {
            ...
            if (hasKey) {
                if (
                    ...
                    this.selectedProducts.forEach((el, index) => {
                        if (
                            ...
                            this.purchase.product_combination_id.splice(
                                index,
                                1
                            );
                ...
                this.combination_product = this.combination_products.filter(
                    function(el) {
